...
class HelpParser:
    def __init__(self):
        self.parser = argparse.ArgumentParser(
            description="Hasat.py: A tool for subdomain enumeration using various tools like subfinder, assetfinder, amass, and bruteforce."
        )
    ...
    def print_help_examples(self):
        ...
        print("   python hasat.py teslamotors.com --type subfinder,assetfinder,amass --bruteforce -w resolvers/output_part_1.txt --rate-limit 100 --rate-limit-trusted 500 --onefile\n")
        print("2. Run with subdomain enumeration tools (subfinder, assetfinder) without bruteforce:\n")
        print("   python hasat.py teslamotors.com --type subfinder,assetfinder\n")
        print("3. Run with subdomain enumeration tools (subfinder, amass) and bruteforce:\n")
        print("   python hasat.py teslamotors.com --type subfinder,amass --bruteforce -w resolvers/output_part_1.txt --rate-limit 100 --rate-limit-trusted 500\n")
        print("4. Run with bruteforce only (without any other tools):\n")
        print("   python hasat.py teslamotors.com --bruteforce -w resolvers/output_part_1.txt --rate-limit 100 --rate-limit-trusted 500\n")
...
class SubdomainScanner:
    ...
    def virus_total_scan(self):
        ...
        print(f"Toplam {len(all_subdomains)} subdomain bulundu ve '{self.domain}_virustotal_subdomains.txt' dosyasına kaydedildi.")
...
$$ |                                     $$ |                            
$$$$$$$\   $$$$$$\   $$$$$$$\  $$$$$$\ $$$$$$\        $$$$$$\  $$\   $$\ 
$$  __$$\  \____$$\ $$  _____| \____$$\\_$$  _|      $$  __$$\ $$ |  $$ |
$$ |  $$ | $$$$$$$ |\$$$$$$\   $$$$$$$ | $$ |        $$ /  $$ |$$ |  $$ |
$$ |  $$ |$$  __$$ | \____$$\ $$  __$$ | $$ |$$\     $$ |  $$ |$$ |  $$ |
$$ |  $$ |\$$$$$$$ |$$$$$$$  |\$$$$$$$ | \$$$$  |$$\ $$$$$$$  |\$$$$$$$ |
\__|  \__| \_______|\_______/  \_______|  \____/ \__|$$  ____/  \____$$ |
                                                     $$ |      $$\   $$ |
                                                     $$ |      \$$$$$$  |
                                                     \__|       \______/ 
    ...
    def run(self):
        self.print_banner()
        print("Welcome to Hasat.py!")

        if len(sys.argv) < 3:
            print("Usage: python hasat.py <domain> --type <tools> [--onefile]")
            print("Example: python hasat.py example.com --type amass,subfinder,assetfinder,gitdork --onefile")
            sys.exit(1)
